...
## 🧪 4. 하위 호환성 및 회귀 방지 검증 (실수 방지 가이드)
* 리포트 기능은 5가지 종류(주간, 격주, 월간, 읽기트레이닝, 리드인)의 반환 JSON 규격과 렌더링 방식이 각각 다릅니다.
* 새로운 템플릿을 연동하거나 코드를 변경할 때는 **반드시 기존 템플릿(나머지 4종)의 렌더링과 데이터 필드들이 정상적으로 매핑되어 작동하는지 3각 교차 분석**을 수행하십시오.
* 렌더링 함수(`renderAiReportPreview`) 수정 시, 아래의 2대 응답 구조가 모두 정상 동작하는지 테스트용 Mock 데이터 시뮬레이션을 필수로 거치십시오.
  1. **신형 구조 (Lidin)**: `sections` 배열 기반 렌더링
  2. **구형 구조 (일반)**: `levelCourse`, `comprehensionText` 등 개별 플랫 키 기반 렌더링
