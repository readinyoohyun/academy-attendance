...
4. 문체는 "~하였습니다.", "~이 필요합니다.", "~을 권장합니다." 와 같이 학부모에게 신뢰감과 권위를 주는 정중하고 전문적인 어조(존댓말)여야 합니다.
   ...
   - 만약 리포트 형식이 '읽기 트레이닝 단독 분석 리포트' 인 경우:
     * title: "읽기트레이닝 [N]권 분석리포트" (예: 읽기트레이닝 6권 분석리포트) 형태로 작성하세요. [N] 부분은 판독된 교재 권수로 채우세요.
      * introMessage: 학부모가 첫머리에서 학생의 정확한 정량 지표를 파악할 수 있도록, 상단의 두 그래프('지문 이해 성취 자료' 및 '읽기 단계별 행동 분석')의 모든 수치 데이터(지문 이해도, 평소 읽기속도, 처음 읽기 속도, 문제 확인 시간, 다시 읽기 시간, 목표 속도)를 문장 내에 직접 텍스트로 풀어 설명하고 그 연관성을 진단하는 풍부하고 구체적인 '종합 데이터 판독 총평'을 서술해 주세요. 단순 완료 알림이나 상투적 인사는 전면 배제하고, 줄 수 제한을 두지 말고 그래프 속 숫자들을 꼼꼼하게 텍스트로 기술해 주어야 합니다. (주의: 형제이름 언급 금지)

     * sections 배열:
       1. title: "1. 학습 현황 보고" -> 이미지와 시트 데이터에서 판독된 완료 권수, 총 차시 수, 소요시간 및 획득한 별 개수 등 정량적인 학습 결과를 구체적으로 보고.
       2. title: "2. 읽기 분석표 해석 기준" -> 빨간색(처리속도 지연), 초록색(평균 수준으로 양호), 회색(평균보다 빠름) 등의 해석 기준 요약. (이 부분은 하드코딩 템플릿과 연동되므로 고정 요약글로 작성)
       3. title: "3. 세부 분석 결과" -> 이미지 속 '분당글자수(WPM) 그래프' 및 '구간별 누적 학습시간 그래프', '학습이력 테이블'을 직접 읽어서 처음읽기 속도와 다시읽기 속도의 차이, 그리고 문제확인 단계에서의 속도 행동 패턴을 구체적인 숫자를 인용하여 정밀 진단 및 해결책 처방. (주의: 비문학 독해, 빠작, PT, 원고지 글쓰기 지도 등 읽기 트레이닝과 직접적으로 무관한 교육 내용은 절대 언급하지 마세요.)
   ...
   - 그 외 형식(주간, 격주, 월간 레벨조정)인 경우:
     ...
     * introMessage: 학부모가 확인하자마자 학생의 종합 성과를 직관적으로 파악할 수 있도록, 이번 분석 기간의 주요 정량 지표(이해도, 어휘/사실/추론 점수 등)를 직접 명시하여 작성한 3~4줄 내외의 데이터 기반 '종합 학업 성취 요약 진단 및 처방글'로 작성해 주세요. 단순 가벼운 인사말이나 완료 안내는 전면 배제하세요.
     * sections 배열:
