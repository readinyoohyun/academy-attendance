...
4. 문체는 "~하였습니다.", "~이 필요합니다.", "~을 권장합니다." 와 같이 학부모에게 신뢰감과 권위를 주는 정중하고 전문적인 어조(존댓말)여야 합니다.
   ...
   - 만약 리포트 형식이 '읽기 트레이닝 단독 분석 리포트' 인 경우:
     ...
     * sections 배열:
       1. title: "1. 학습 현황 보고" -> 이미지와 시트 데이터에서 판독된 완료 권수, 총 차시 수, 소요시간 및 획득한 별 개수 등 정량적인 학습 결과를 구체적으로 보고. (주의: 읽기트레이닝은 1권당 총 10강(10차시)으로 구성되므로 절대 '60강' 등으로 확대 왜곡하지 말고 해당 1권 과정인 '총 10강 과정 완료' 또는 '10개 차시 완료'라고 정확하게 서술하세요.)

       2. title: "2. 읽기 분석표 해석 기준" -> 빨간색(처리속도 지연), 초록색(평균 수준으로 양호), 회색(평균보다 빠름) 등의 해석 기준 요약. (이 부분은 하드코딩 템플릿과 연동되므로 고정 요약글로 작성)
       3. title: "3. 세부 분석 결과" -> 이미지 속 '분당글자수(WPM) 그래프' 및 '구간별 누적 학습시간 그래프', '학습이력 테이블'을 직접 읽어서 처음읽기 속도와 다시읽기 속도의 차이, 그리고 문제확인 단계에서의 속도 행동 패턴을 구체적인 숫자를 인용하여 정밀 진단 및 해결책 처방. (주의: 비문학 독해, 빠작, PT, 원고지 글쓰기 지도 등 읽기 트레이닝과 직접적으로 무관한 교육 내용은 절대 언급하지 마세요. 총 학습량은 60강이 아니라 해당 권수 10강 기준임에 유의하세요.)
