...
class CRMManager {
  ...
  async generateAiReport() {
    ...
    try {
      ...
      const queryKeywords = ["리드인리포트", "리드인분석표", "리포트", "분석표"];
      const isLidinReport = (text) => {
        const normalizedText = String(text || "").replace(/\s+/g, "");
        return queryKeywords.some(kw => normalizedText.includes(kw));
      };
      
      const studentConsultations = (this.app.state.consultations || []).filter(c => {
        return c && c.name && c.name.replace(/\s+/g, '') === name.replace(/\s+/g, '');
      });
      
      const pastLidinReports = studentConsultations.filter(c => {
        return isLidinReport(c.author) || isLidinReport(c.content);
      });
      
      let pastReportsReference = "";
      if (pastLidinReports.length > 0) {
        pastReportsReference = pastLidinReports.slice(-3).map((r, idx) => {
          return `--- 과거 리포트 예시 ${idx + 1} ---\n[작성기간]: ${r.period || '없음'}\n[내용]:\n${r.content || ''}`;
        }).join("\n\n");
      } else {
        pastReportsReference = "등록된 과거 리포트 이력 없음 (원장의 기본 리포트 작법 스타일로 집필하세요)";
      }
      ...
      userParts.push({
        text: (() => {
          if (cycle === 'lidin_report') {
            return `역할: 당신은 한국의 최고급 독서 지도 학원의 원장이자 전문 독서코치입니다. 학부모님께 전달할 격식 있고 매우 정중하며 정교한 리드인 독서 활동 리포트를 집필해 주세요.
...
4. sections 배열 (반드시 다음 6대 영역 제목과 지침에 맞춰서 집필해 주세요):
  ...
  - title: "4. 글쓰기 및 문장 구성력"
    * 지침: 독서 후 책과 관련된 글짓기(2~3문장 표현력), 또는 어휘를 활용해 본인만의 상황에 어울리는 문장을 스스로 자연스럽게 만드는 능력을 구체적 문장 인용 예시와 함께 서술해 주세요.
  ...
  "sections": [
   ...
   "stats": {
    ...
    "critique": ${critiqueVal}
          ...
          return `역할: 당신은 한국의 최고급 독서 지도 학원의 원장이자 전문 독서코치입니다. 학부모님께 전달할 격식 있고 매우 정중하며 정교한 학업 성취 종합 분석 성적표를 집필해 주세요.
...
[수행 지시 사항 (중요! 정밀 분석 및 구체적 집필 지침)]:
1. 첨부된 이미지들(리드인 종합분석결과 그래프, WPM 분당 글자수 추이 곡선, 구간별 누적 학습시간 그래프, 차시별 빨강/초록/회색 동그라미 학습이력 테이블 등)의 모든 수치와 그래프를 꼼꼼하게 판독(OCR 및 시각 분석)하세요.
2. 분석글 작성 시 "열심히 학습했습니다"와 같은 두루뭉술하고 상투적인 표현은 배제하고, 반드시 이미지 및 연동 데이터에서 판독된 구체적인 수치(예: 평소 읽기속도 433자, 처음읽기 평균 380자, 다시읽기 476자, 소요시간 14분 6초, 이해도 97% 등)를 문장 내에 직접 명시하여 정밀 분석을 제공해야 합니다.
3. 중요 지침 (내용의 풍부함과 여백 조절): 
   - 문장이 너무 축약되거나 짧으면 정보가 부실하여 신뢰감이 떨어지므로, 각 section의 text는 핵심 데이터 분석과 전문적인 독서 교정 처방을 결합하여 4~5줄 내외(200자~280자 내외)로 내실 있게 집필해 주세요.
   - 단, 1페이지 인쇄 범위를 벗어나지 않도록 너무 장황한 미사여구는 줄이고 정보의 밀도를 극대화해 주세요.
4. 문체는 "~하였습니다.", "~이 필요합니다.", "~을 권장합니다." 와 같이 학부모에게 신뢰감과 권위를 주는 정중하고 전문적인 어조(존댓말)여야 합니다.
   ...
   - 만약 리포트 형식이 '읽기 트레이닝 단독 분석 리포트' 인 경우:
     * title: "읽기트레이닝 [N]권 분석리포트" (예: 읽기트레이닝 6권 분석리포트) 형태로 작성하세요. [N] 부분은 판독된 교재 권수로 채우세요.
       * introMessage: "[이름]이가 읽기트레이닝 [N]권 과정을 성공적으로 완료했습니다. 다음 등원 시 정성스럽게 작성된 교재와 세부 분석표, 그리고 이 분석 리포트를 전달하겠습니다. 가정에서도 우리 [이름]이가 한 권의 과정을 무사히 마친 것에 대해 많은 칭찬과 격려를 부탁드립니다. 늘 믿고 맡겨주셔서 감사드립니다!" 형태로 학부모에게 보내는 정중하고 따뜻하며 격려가 담긴 인사말로 작성해 주세요. (주의: 형제이름 언급 및 기타 사족 절대 금지, 분량은 3줄 내외)

     * sections 배열:
       1. title: "1. 학습 현황 보고" -> 이미지와 시트 데이터에서 판독된 완료 권수, 총 차시 수, 소요시간 및 획득한 별 개수 등 정량적인 학습 결과를 구체적으로 보고. (주의: 읽기트레이닝 6권은 총 10강(10차시) 과정입니다. 이미지 속에 적혀 있는 '총 60강'은 1권부터 6권까지의 모든 학습 이적이 누적된 전체 풀이 수이므로, 리포트에는 이번 과정의 실질적 강의 수인 '10강 완료' 또는 '10개 차시 완료'라고 정확하게 반영해야 하며 절대 '60강'을 이번 학기 학습 수로 오인하여 서술하지 마세요.)

       2. title: "2. 읽기 분석표 해석 기준" -> 빨간색(처리속도 지연), 초록색(평균 수준으로 양호), 회색(평균보다 빠름) 등의 해석 기준 요약. (이 부분은 하드코딩 템플릿과 연동되므로 고정 요약글로 작성)
       3. title: "3. 세부 분석 결과" -> 이미지 속 '분당글자수(WPM) 그래프' 및 '구간별 누적 학습시간 그래프', '학습이력 테이블'을 직접 읽어서 처음읽기 속도와 다시읽기 속도의 차이, 그리고 문제확인 단계에서의 속도 행동 패턴을 구체적인 숫자를 인용하여 정밀 진단 및 해결책 처방. (주의: 비문학 독해, 빠작, PT, 원고지 글쓰기 지도 등 읽기 트레이닝과 직접적으로 무관한 교육 내용은 절대 언급하지 마세요. 총 학습량은 누적 수치인 60강이 아니라 이번 학기 교재 1권에 해당하는 10강 기준임에 유의하고, 두 번째 사진(종합분석결과)의 획일적인 '총평 코멘트'를 그대로 베껴 쓰지 마세요. 오직 첫 번째 사진(학습이력 테이블)에 나타난 10강의 각 항목별 등급 분포와 처음읽기(380자), 다시읽기(476자) 등의 구체적인 데이터를 바탕으로 개별적이고 고도화된 정밀 분석을 집필해야 합니다.)

       4. title: "4. 향후 학습 계획" -> 승급할 다음 권수(예: 7권) 및 메타인지/자기점검 향상을 위한 구체적인 1:1 상담 집중 지도 계획 기술. (주의: 다른 수업 계획은 절대 지어내거나 언급하지 마세요).
